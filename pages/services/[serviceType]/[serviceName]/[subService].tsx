...
export default function SubService(props: ReactNode & { page: Page }) {
    const { page } = props;
    const { content } = page;
    ...
    return router.isFallback ? <>Loading...</> : (
        <div className={styles.serviceName}>
            ...
            {
                content?.headerWithContent &&
                <HeaderWithContent
                    headerWithContent={content.headerWithContent}
                />
            }
            {
                content.header &&
...
export async function getStaticProps(props: Params) {
    ...
    const data = await getPage(`/services/${params?.serviceType}/${params.serviceName}/${params.subService}`);
    return {
        props: {
            page: data ?? null,
        }
...
export async function getStaticPaths() {
    ...
    return {
        paths: allPages?.map((page) => `${page?.slug}`).filter(x => x.split("/").length === 5),
        fallback: false
    }
}